...
import { advanceOnce, advanceToPlayer, run, testState, player, unit } from './helpers';
...
describe('§5.1 主迴圈', () => {
  it('永遠挑 nextActAt 最小的單位行動', () => {
    const s = testState(OPEN, [{ archetype: 'RUNNER', pos: { x: 6, y: 1 } }]);
  ...
  it('每個單位輪到時必須做一件事：IDLE 敵人也花時間，不會卡住排程器', () => {
    let s = testState(OPEN, [{ archetype: 'RUNNER', pos: { x: 12, y: 3 } }]);
  ...
  it('動作效果立即結算，nextActAt 隨後推進', () => {
    let s = testState(OPEN, [{ archetype: 'RUNNER', pos: { x: 4, y: 1 } }]);
