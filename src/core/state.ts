 ...
 * **「餵幾種彈藥」**而不是「哪把槍比較強」。
 */
export type Calibre = '5.56' | '7.62' | '12ga' | '9mm' | '84mm' | 'melee';
...
export interface WeaponStats {
  ...
  action: WeaponAction;
  /**
   * 補給基準（v0.18 附錄）：回到公司時自動補到「幾個彈倉的量」。
   * 省略時視為 1。這只是局外層的便利設定，戰術層完全不看它。
   */
  resupplyMagazines?: number;
  /**
   * 遺產（撤離前製造，不可補充）或土製（現在還做得出來）。
   * 分界線是**自動循環**（v0.15 附錄 B §20.3）。決定價格級距與供給方式。
   */
  origin?: 'LEGACY' | 'LOCAL';
  /**
   * **內建武器**（§1.2）：長在身上的那一件，不是撿來的。
   *
   * 射程 1、彈藥無限、重量 0、只有單發，**不佔主手也不佔收納欄**，
   * 不可丟棄、不可掉落、不進軍械庫。衝鋒型的爪、裝甲型的重擊、
   * 士兵的工兵刀走的是同一條解算路徑 —— 它們不是特例，是武器。
   *
   * 存在的理由：彈藥現在是有限資源，打光的單位必須還有事可做。
   */
  intrinsic?: boolean;
  /**
   * **體積大到藏不住**（§4.2）：從任何距離都看得見，而且開火要先架設（§4.4）。
   *
   * 這是「一發即死沒問題，看不到它要來才有問題」的實作方式 ——
   * 不排除任何武器，改為讓威脅可見、可預測、可應對。
   */
  conspicuous?: boolean;
...
export interface Unit {
  ...
  /**
   * 內建近戰武器（§1）。**永遠存在，不佔任何欄位。**
   *
   * 主手打不出去（沒彈、超出射程）而目標就在旁邊時，攻擊自動改用它 ——
   * 不需要切換動作，也不需要新的按鈕。
   */
  intrinsic: Weapon;
  /**
   * 彈匣外還帶著幾發（§3.2）。**只有敵人用得到** ——
   * 玩家的備彈在背包裡，那是一個可以整理、可以搬動、可以丟棄的東西。
   *
   * 敵人不做裝備管理，所以他們的備彈只是一個數字：打完就沒了，
   * 沒了就改用內建近戰（§3.4）。
   */
  reserveAmmo: number;
  /**
   * 顯眼武器已經架好了，下一個動作就是那一發（§4.4）。
   *
   * **被打斷即作廢**：宣告失效時連同這個一起清掉，已花的時間不退還。
   * 這是玩家看到砲口轉過來之後，那一個完整行動的價值所在。
   */
  setUp: boolean;
...
export type DeclKind =
  ...
  | 'RELOAD'
  | 'SEARCH_MOVE'
  /** 架設顯眼武器（§4.4）。架好之後的下一個動作才是那一發。 */
  | 'SETUP'
  /** 彈盡，改用內建近戰（§3.3）。「換彈」與「沒子彈了」都是真的戰術訊號。 */
  | 'NO_AMMO'
...
export interface GameState {
   ...
   * 已識別的武器實例 id（§4.2）。
   *
   * 三個層級：**未識別**只看得到敵人、**已識別**武器圖示常駐、
   * **恆常可見**（`conspicuous`）不需要識別。
   *
   * 識別條件是「該敵人開過火」或「雙方距離 ≤ 門檻」，一旦識別就在這一場裡保留。
   * 它進 `GameState` 而不是渲染狀態，因為口令能不能說出武器名稱要看它（§4.3）——
   * 否則玩家可以靠聽的繞過整個識別系統。
   */
  identifiedWeapons: string[];
  rngSeed: number;
  rng: RngState;
