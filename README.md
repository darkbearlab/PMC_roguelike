...
規格書：[`clone-pmc-roguelike-mvp-spec.md`](clone-pmc-roguelike-mvp-spec.md)（v0.6，與實作同步）

```bash
npm install
npm run dev        # http://localhost:5173  （手機同網段可用 --host 顯示的網址開）
npm run test       # 185 個測試：core/ 規則層、UI 互動文法、整張正式地圖的整合測試
npm run typecheck
npm run build
...
**煞車必須是彈藥**。一個彈匣打兩回合，四發在 55% 下平均命中 2.2 發。
「這一發值不值得」因此成為每回合都要問的問題 —— 剛好落在世界觀那句話上：子彈比人貴。
...
敵人從 `IDLE` 轉入 `ALERT` 的那個回合**不得攻擊**，只能轉向與移動。
這一回合正是玩家用來蹲下、退回掩體或先開槍的窗口。
